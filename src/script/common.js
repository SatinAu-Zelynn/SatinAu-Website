...
/* ========== 鼠标滚轮惯性滚动 (Linear Slowing Down) ========== */
class MomentumScroll {
  constructor() {
    this.bindMethods();
    
    this.data = {
      current: window.scrollY, // 当前视觉上的滚动位置
      target: window.scrollY,  // 目标滚动位置
      ease: 0.15,              // 缓动系数 (0.01 - 1)，越小越顺滑，惯性越大
      isScrolling: false       // 标记是否正在进行惯性滚动
    };
  ...
  onWheel(e) {
    // 如果按住 Ctrl (缩放) 或在多行文本框内，不拦截
    if (e.ctrlKey || e.target.tagName === 'TEXTAREA') return;

    e.preventDefault();

    // 累加滚动距离
    // e.deltaY 在不同浏览器可能数值差异大，这里做一个简单的标准化
    let delta = e.deltaY;
    
    // Firefox 的 deltaY 单位通常是“行”，数值很小，需要放大
    ...
    const maxScroll = Math.max(
      ...
      this.dom.html.scrollHeight,
      this.dom.body.offsetHeight, 
      this.dom.html.offsetHeight, 
      this.dom.body.clientHeight, 
      this.dom.html.clientHeight
    ) - window.innerHeight;

    this.data.target = Math.max(0, Math.min(this.data.target, maxScroll));
  }

  // 处理原生滚动事件（例如用户拖动滚动条）
  onScroll() {
    // 如果当前不是由滚轮驱动的惯性滚动中，说明用户用了其他方式滚动
    // 此时需要立即同步 target，防止下次滚轮时跳回旧位置
  ...
  update() {
    ...
    // 四舍五入保留3位小数，避免极小数值抖动
    this.data.current = parseFloat(this.data.current.toFixed(3));

    // 计算滚动的距离差
    const diff = Math.abs(this.data.target - this.data.current);

    // 如果距离很小，直接吸附到目标位置并停止计算，节省性能
    if (diff < 0.5) {
    ...
    // 只有当数值不一致时才执行滚动，减少重绘
    if (this.data.isScrolling || Math.abs(window.scrollY - this.data.current) > 1) {
...
// 页面加载完成后初始化
document.addEventListener('DOMContentLoaded', () => {
  window.momentumScrollInstance = new MomentumScroll();
});